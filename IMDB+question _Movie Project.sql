...
SELECT director_name,movie_count, director_row_rank FROM top_director
...
SELECT T.actor_name, 
SUM(T.total_votes) as total_votes,
COUNT(T.id) as movie_count,
ROUND(SUM(T.avg_rating*T.total_votes)/SUM(T.total_votes),2) AS actor_avg_rating,
RANK() OVER(ORDER BY COUNT(T.id) DESC) AS actor_rank
FROM 
(SELECT t1.id, t2.avg_rating, t2.total_votes, t4.name as actor_name FROM movie t1

JOIN ratings t2
ON t1.id=t2.movie_id
JOIN role_mapping t3
ON t2.movie_id = t3.movie_id
JOIN names t4
ON t3.name_id = t4.id
WHERE category='actor' AND country= 'india')T
GROUP BY T.actor_name
HAVING COUNT(T.id)>=5
LIMIT 1;
...
SELECT T.actress_name, 
SUM(T.total_votes) as 'total_votes',
COUNT(T.id) as movie_count,
ROUND(SUM(T.avg_rating*T.total_votes)/SUM(T.total_votes),2) AS actor_avg_rating,
RANK() OVER(ORDER BY T.actress_name DESC) AS 'actress_rank'
FROM (SELECT t1.id, t2.avg_rating, t2.total_votes, t4.name as 'actress_name'
FROM movie t1

INNER JOIN ratings t2
ON t1.id=t2.movie_id
INNER JOIN role_mapping t3
ON t2.movie_id = t3.movie_id
INNER JOIN names t4
ON t3.name_id = t4.id
WHERE category='actress' AND country= 'india' AND languages='hindi')T
group by T.actress_name
HAVING COUNT(T.id)>=3
LIMIT 1;
